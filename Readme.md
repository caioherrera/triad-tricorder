...
# TriAD (Ferramenta de Detecção de Anomalias baseada na Tricorder)

## Introdução

Esta pasta contem os arquivos relativos à ferramenta TriAD. Esta ferramenta possui 4 módulos, que podem ser executados em conjunto por meio do `Docker compose`, ou individualmente:
...
- *Tricorder-UI*: projeto frontend que fornece uma UI para uso da ferramenta TriAD. 
...
## Instruções para execução dos experimentos da TriAD dentro do container Docker
...
`git clone https://github.com/caioherrera/triad-tricorder.git`

2) Rode a imagem da TriAD. Para construir e executar com base no Dockerfile, add --detach para execução em segundo plano.:
...
# TriAD (Tricorder-based Anomaly Detection Tool)

## Introduction

This folder contains the files related to the TriAD tool. The tool has 4 modules, which can be run together using `Docker compose`, or individually:
...
- *Tricorder-UI*: frontend project that provides a UI for using the TriAD tool.
...
## Instructions for running TriAD experiments inside the Docker container
...
2) Run the TriAD image. To build and run based on the Dockerfile, add --detach to run in the background:
